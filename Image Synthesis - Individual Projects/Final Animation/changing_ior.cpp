...
void setPixels()
{
	...
	Sphere *sp=new Sphere(-40,20,-20,40,sph2_c,true,true,'s',2.66);
	objects[ind]= sp;
	ind+=1;
	...
	for(int f=0;f<48;f++)
	{
		...
		for (int y = 0; y < height; y++) 
		{
			for (int x = 0; x < width; x++) 
			{
				...
				plane_arr_f[i] = final_col.x/25;
				plane_arr_f[i+1] = final_col.y/25;
				plane_arr_f[i+2] = final_col.z/25;

					
			}
		}
		stbi_flip_vertically_on_write(true);
		std::string filename="changing_ior/sphere-fresnel"+std::to_string(f)+".jpg";
		stbi_write_jpg(filename.c_str(),200,200,3,plane_arr_f,100);
	}
}
