...
Vector castRay(Vector &Pe, Vector &npe, Vector &default_col,Vector &Pl,float &T, float &Sp,const int &depth=0)
{
	float kr;
...
void setPixels()
{
	...
	Vector L2=cross_product(L0,L1);


	Vector L_n0=L0*(1/magnitude(L0.x,L0.y,L0.z));
	Vector L_n1=L2*(1/magnitude(L2.x,L2.y,L2.z));



	float L_s0=30,L_s1=30;

	Vector L00=Lc-(L_n0*(L_s0/2))- (L_n1*(L_s1/2));
	...
	for (int y = 0; y < height; y++) 
	{
		for (int x = 0; x < width; x++) 
		{
			...
			for(float k=0;k<m;k++)
			{
				for (float l=0;l<n;l++)
				{
					Vector L=L00+L_n0*L_s0*(l/n)+L_n1*L_s1*(k/m);

					final_col=castRay(Pe,npe,default_col,L,T,S);
					Tf+=T;
					Sf+=S;
				}
			}
