...
void setPixels()
{
	...
	Sphere *sp=new Sphere(50,-30,0,50,sph1_c,false,false,'s',1);
	objects[ind]= sp;
	ind+=1;

	sp=new Sphere(-50,20,0,50,sph2_c,false,false,'s',1);
	objects[ind]= sp;
	ind+=1;

	sp=new Sphere(150,-80,0,50,sph3_c,false,false,'s',1);
	objects[ind]= sp;
	ind+=1;
	...
	std::uniform_int_distribution<int> sampling(0,5);
				...
				for (int x = 0; x < width; x++) 
				{
					...
					for(int m=0;m<M;m++)
					{
						for(int n=0;n<N;n++)
						{
							...
							final_col=final_col+castRay(P_eye_sample ,npe,default_col);
					...
					result[i] = final_col.x / (M*N);
					result[i+1] = final_col.y/ (M*N);
					result[i+2] = final_col.z/ (M*N);
