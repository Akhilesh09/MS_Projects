...
void setPixels()
{
	...
	Sphere *sp=new Sphere(0,0,0,50,sph1_c,false,false,'s',1);
	objects[ind]= sp;
	ind+=1;
	...
	int frame_start=-25,frame_end=25;
	int time_samples=15;

	std::default_random_engine generator;
	std::uniform_int_distribution<int> frames(frame_start,frame_end);
			...
			for (int y = 0; y < height; y++) 
			{
				for (int x = 0; x < width; x++) 
				{
					...
					for(int sample=0;sample<time_samples;sample++)
					{
						T=frames(generator); // random frame between -25 to 25
					...
					result[i] = final_col.x /time_samples;
					result[i+1] = final_col.y/time_samples;
					result[i+2] = final_col.z/time_samples;
