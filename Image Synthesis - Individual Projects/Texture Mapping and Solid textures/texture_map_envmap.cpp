...
int width = 100, height = 100, channels1, channels2, channels3;
int width1 = 600, height1 = 600;
class Vector
{
public:
	float x,y,z;
...
void setPixels()
{
	...
	env_img = stbi_load("night_sky.jpg", &width1, &height1, &channels1, STBI_rgb);
	for (int y = 0; y < 600; y++) 
	{
		for (int x = 0; x < 600; x++) 
		{
		...
		for (int x = 0; x < width; x++) 
		{
			...
			if(b>0 || isnan(disc))
			{
				...
				float t_hit = -b + disc;
					...
					float v = acos(s_n2 * (P_hit - Pc) / 600) / 3.14;
					float u = acos(s_n1 * (P_hit - Pc) / 600 / sin(3.14 * v)) / (2 * 3.14);

					float x_prime = v * height1;
					float y_prime = u * width1;

					if (s_n0 * (P_hit - Pc) / 600 < 0)
						u = 1 - u;

				int k=((int)y_prime*width1+(int)x_prime)*3;
				result[i] = env_arr[k];
				result[i+1] = env_arr[k+1];
				result[i+2] = env_arr[k+2];
			}
			else
			{
...
int main(int argc, char *argv[])
{
	...
	width = 100;
	height = 100;
	result = new unsigned char[100 * 100 * 3];
	texture_arr = new unsigned char[100 * 100 * 3];
