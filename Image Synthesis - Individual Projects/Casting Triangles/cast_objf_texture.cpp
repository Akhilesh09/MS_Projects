	...
	if (loadout)
	{
		...
		for (int i = 0; i < Loader.LoadedMeshes.size(); i++)
		{
			...
			for (int j = 0; j < curMesh.Vertices.size()-3; j+=3)
			{
				...
				objl::Vector3 Shading_N= curMesh.Vertices[j].Normal*s + curMesh.Vertices[j+1].Normal*t + curMesh.Vertices[j+2].Normal*u;
				objl::Vector2 Texture_C= curMesh.Vertices[j].TextureCoordinate*s + curMesh.Vertices[j+1].TextureCoordinate*t + curMesh.Vertices[j+2].TextureCoordinate*u;
				...
				// plane_arr_f[l] = 255*(1-T)+0*(T);
				// plane_arr_f[l+1] = 0*(1-T)+0*(T);
				// plane_arr_f[l+2] = 0*(1-T)+255*(T);
				}
				}
				}
				}
