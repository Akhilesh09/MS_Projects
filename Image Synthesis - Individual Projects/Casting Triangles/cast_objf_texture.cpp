...
int width = 200, height = 200, channels1;
...
public:
	...
	bool intersect(Vector& P, Vector& dir, float& t_hit)
	{
		...
		if (num < 0 && den < 0)
		{
			...
			w = (n * T_A0) / magnitude(A.x, A.y, A.z);


			if (s > 0 && s < 1 && t>0 && t < 1 && w>0 && w < 1)
			{
				u = s;
				v = t;
				return true;
...
Vector castRay(Vector& Pe, Vector& npe, Vector& default_col, const int& depth = 0)
{
	...
	if (trace(Pe, npe, index, t_hit))
	{
		...
		float u= objects[index]->u,v= objects[index]->v;
		...
		hit_col.x = texture_arr[k];
		hit_col.y = texture_arr[k + 1];
		hit_col.z = texture_arr[k + 2];
