...
void GetColor(
	...
	D = (1.0 / (pow(SphCent[SphNum].m, 2.0) * pow(cos(alpha), 4))) * exp(-1.0 * pow(tan(alpha) / SphCent[SphNum].m, 2.0));
	...
	R = 3.58 * X - 1.504 * Y - 0.55 * Z, G = -1.18 * X + 1.988 * Y + 0.601 * Z, B = 0.063 * X - 0.193 * Y + 0.869 * Z; // Extra credit using Monitor specific values
	clamp(R, 0.0, 1.0);
	clamp(G, 0.0, 1.0);
	clamp(B, 0.0, 1.0);
