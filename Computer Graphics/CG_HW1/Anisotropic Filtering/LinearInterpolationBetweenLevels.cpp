...
void setPixels(string filterType)
{
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++)
		{
			...
			if (num < 0 && den < 0)
			{
				...
				//linear interpolation between the 2 levels
				int index_lower = (int(v * height1[d_lower]) * width1[d_lower] + int(u * width1[d_lower])) * 3;
				int index_higher = (int(v * height1[d_higher]) * width1[d_higher] + int(u * width1[d_higher])) * 3;


				result[i] = (d - d_lower) * texture_img[d_lower][index_lower] + (d_higher - d) * texture_img[d_higher][index_higher];
				result[i + 1] = (d - d_lower) * texture_img[d_lower][index_lower + 1] + (d_higher - d) * texture_img[d_higher][index_higher + 1];
				result[i + 2] = (d - d_lower) * texture_img[d_lower][index_lower + 2] + (d_higher - d) * texture_img[d_higher][index_higher + 2];
	...
	stbi_write_jpg("anistropic_filter_LI.jpg", width, height, 3, result, 100);
}
