...
class Kernel_Layer(nn.Module):
    ...
    def _k_means(self, X):
        ...
        centroids = KMeans(self.hidden_dim).fit(X).cluster_centers_
    ...
    def forward(self, x):
        ...
        if torch.cuda.is_available():
            ...
            protos=torch.tensor(self.prototypes.cpu().numpy())

        

        k_values=torch.zeros((x.shape[0],self.prototypes.shape[0]))
        for i in range(x.shape[0]):
            for j in range(self.prototypes.shape[0]):
                k_values[i][j]=np.exp(((np.linalg.norm(x[i]-protos[j])**2)*-1)/(2*(self.sigma**2)))
