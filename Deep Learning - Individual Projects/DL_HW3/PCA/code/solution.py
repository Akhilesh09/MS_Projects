...
class PCA():
    ...
    def _do_pca(self):
        ...
        ones_matrix=torch.ones((X.shape[1],1),dtype=torch.double)
        X_mean=torch.div(torch.mm(X,ones_matrix),self.X.shape[1])
        X_centered=(X-torch.mm(X_mean,torch.transpose(ones_matrix,0,1)))
        U, S, V_transp = torch.svd(X_centered)
        Up=U[:,:self.n_components]
        Xp=torch.mm(torch.transpose(Up,0,1),X_centered)
    ...
    def reconstruction(self, Xp):
        ...
        X_re=torch.mm(self.Up,Xp).numpy()
...
class AE(nn.Module):
    ...
    def _network(self):
        ...
        weights1=torch.empty((self.n_features,self.d_hidden_rep))
        nn.init.kaiming_normal_(weights1)
        weights2=torch.empty((self.n_features,self.d_hidden_rep))
        nn.init.kaiming_normal_(weights2)
        self.w1 = nn.Parameter(weights1)
        self.w = nn.Parameter(weights2)
        

        # Note: here for the network with more layers and nonlinear functions  
        self.net = nn.Sequential(
            nn.Linear(self.n_features,self.d_hidden_rep),
            nn.Sigmoid(),
            nn.Linear(self.d_hidden_rep,self.n_features),
            nn.Sigmoid()
        )
    ...
    def _forward(self, X):
        ...
        return (torch.mm(torch.mm(self.w,torch.transpose(self.w,0,1)),X))


        # Note: here for the network without weights sharing 
        return (torch.mm(torch.mm(self.w,torch.transpose(self.w1,0,1)),X))
    ...
    def reconstruction(self, X):
        ...
        X_re = torch.transpose(torch.empty((X.shape)),0,1)
        ...
        with torch.no_grad():
            for i in range(n_samples):
                ...
                X_re[i]=torch.transpose(curr_X_re_tensor,0,1)

            X_re=torch.transpose(X_re,0,1).numpy()
            ### END YOUR CODE 
        return X_re