...
class Config(object):
	...
	embed_size = 200
	hidden_size = 500
	num_steps = 30 # RNN is unfolded into 'num_steps' time steps for training
...
class RNNLM_Model(nn.Module):
	...
	def __init__(self, config):
		...
		self.L=nn.Embedding(num_embeddings=self.config.vocab_size,embedding_dim=self.config.embed_size)

		### Define the H, I, b1 in HW4. Hint: check nn.Parameter
		I=torch.empty((self.config.embed_size,self.config.hidden_size))
		nn.init.kaiming_normal_(I)
		self.I = nn.Parameter(I)

		H=torch.empty((self.config.hidden_size,self.config.hidden_size))
		nn.init.kaiming_normal_(H)
		self.H = nn.Parameter(H)

		b1=torch.empty((1,self.config.hidden_size))
		nn.init.kaiming_normal_(b1)
		self.b1 = nn.Parameter(b1)

		### Define the projection layer, U, b2 in HW4

		U=torch.empty((self.config.hidden_size,self.config.vocab_size))
		nn.init.kaiming_normal_(U)
		self.U = nn.Parameter(U)

		b2=torch.empty((1,self.config.vocab_size))
		nn.init.kaiming_normal_(b2)
	...
	def add_embedding(self, input_x):
		...
		"""
		### YOUR CODE HERE
		input_x=self.L(input_x)
		input_x=torch.reshape(input_x, (self.config.num_steps,self.config.batch_size,self.config.embed_size))
		input_x=list(i for i in input_x)
	...
	def add_model(self, input_x, initial_state):
		...
		for x in input_x:
			next_state=self.output_drop(torch.sigmoid(torch.mm(prev_state,self.H)+torch.mm(x,self.I)+self.b1))
	...
	def add_projection(self, rnn_outputs):
		...
		for i in rnn_outputs:
			outputs.append(torch.mm(i,self.U)+self.b2)
		### END YOUR CODE
		return outputs
...
def compute_loss(outputs, y, criterion):
	...
	x=torch.zeros((num_steps,batch_size,vocab_size))
	for i in range(num_steps):
		x[i]=outputs[i]
	x=torch.reshape(x,(num_steps*batch_size,vocab_size)).cuda()
	y=torch.reshape(y,(num_steps*batch_size,)).cuda()
	loss=criterion(x,y)
