...
class Cifar(object):
	...
	def setup(self, training):
		...
		if training:
			...
			### YOUR CODE HERE
			# cross entropy
			cross_entropyloss=tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.labels,logits=logits)
			...
			for v in tf.trainable_variables():
				if 'kernel' in v.name:
					count+=1
			self.losses=tf.reduce_mean(cross_entropyloss)+l2_loss
	...
	def train(self, x_train, y_train, max_epoch):
		...
		for epoch in range(1, max_epoch+1):
			...
			for i in range(num_batches):
				...
				x_batch = np.asarray([parse_record(i,True) for i in x_batch])
	...
	def test_or_validate(self, x, y, checkpoint_num_list):
		...
		for checkpoint_num in checkpoint_num_list:
			...
			for i in tqdm(range(x.shape[0])):
				...
				# preds.append(tf.arg_max(network(x_test,False)))
				feed_dict = {self.inputs: x_test[None,:]}
